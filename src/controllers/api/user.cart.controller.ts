...
import { AddCarpetToCartDto } from "src/dtos/cart/add.carpet.to.cart.dto";
import { EditCarpetInCartDto } from "src/dtos/cart/edit.carpet.in.cart.dto";
...
export class UserCartController {
    ...
    @UseGuards(RoleCheckedGuard)
    @AllowToRoles('user')
    async addToCart(@Body() data: AddCarpetToCartDto, @Req() req: Request): Promise<Cart> {
        const cart = await this.getActiveCartForUserId(req.token.id);
        return await this.cartService.addCarpetToCart(cart.cartId, data.carpetId, data.quantity);
    }
    ...
    @UseGuards(RoleCheckedGuard)
    @AllowToRoles('user')
    async changeQuantity(@Body() data: EditCarpetInCartDto, @Req() req: Request): Promise<Cart> {
        const cart = await this.getActiveCartForUserId(req.token.id);
        return await this.cartService.changeQuantity(cart.cartId, data.carpetId, data.quantity);
    }
